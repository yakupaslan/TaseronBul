...
namespace TaseronBul.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [AllowAnonymous]
        public ActionResult Login2(Uye uye)
        {
            string mail = uye.Mail;
            string sifre = uye.Sifre;

            var user1 = (from u in db.Uyes where u.Mail == mail select u).FirstOrDefault();
            if (user1 != null && (user1.Mail == mail && user1.Sifre == sifre))
            {
                FormsAuthentication.SetAuthCookie(mail, true);
